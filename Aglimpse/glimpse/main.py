...
def answer_queries_in_log(KG, K, query_log, summary_methods, test_size=0.5):
    ...
    for summary_method in summary_methods:
        logging.info(
            '\t---Summarizing with {}---'.format(summary_method.name()))
        ...
        total_F1, total_precision, total_recall = total_query_log_metrics(
            S, test_log)
        logging.info('\t  Total F1/precision/recall')
        logging.info('\t    {:.2f}/{:.2f}/{:.2f}'.format(
            total_F1, total_precision, total_recall))

        avg_F1, avg_precision, avg_recall = average_query_log_metrics(
            S, test_log)
...
def main():
    ...
    for user in range(args.n_users):
        ...
        logging.info(
            '---Generated a log of {} queries----'.format(len(query_log)))

        answer_queries_in_log(
            KG, K, query_log, summary_methods, test_size=args.test_size)
